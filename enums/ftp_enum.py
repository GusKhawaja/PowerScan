...
nmap_tool_name = 'NMAP FTP Enum'
hydra_brute = 'Hydra FTP Brute Force'
...
def brute_force(ip_address):
    file_name = './temp/' + ip_address + "_ftp.txt"
    command = 'hydra -t 10 -V -f -L ./resources/common_users.txt -P ./resources/common_passwords.txt ftp://' + ip_address + '>' + file_name
...
def start(ip_address, service):
    ...
    try:
        result = pool.apply_async(execute_nmap_ftp_enum, [ip_address, service.port])
    ...
    try:
        ...
        enum = EnumerationDTO('Check FTP Vulnerabilities')
        enum.results_output = output
        service.enumeration_list.append(enum)
    except TimeoutError:
