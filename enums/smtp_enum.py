...
nmap_tool_name = 'NMAP SMTP Enum'


def execute_nmap_smtp_enum(ip_address, port_number):
    command = "nmap -sV -p %s --script=smtp* %s" % (port_number, ip_address)
...
def start(ip_address, service):
    ...
    try:
        result = pool.apply_async(execute_nmap_smtp_enum, [ip_address, service.port])
    ...
    try:
        ...
        enum = EnumerationDTO('Check SMTP Vulnerabilities')
        enum.results_output = output
        service.enumeration_list.append(enum)
    except TimeoutError:
